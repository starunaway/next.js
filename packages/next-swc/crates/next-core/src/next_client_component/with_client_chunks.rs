...
use turbopack_binding::{
    ...
    turbopack::{
        core::{
            ...
            chunk::{
                availability_info::AvailabilityInfo, Chunk, ChunkData, ChunkItem, ChunkableAsset,
...
impl WithClientChunksChunkItem {
    ...
    #[turbo_tasks::function]
    async fn client_chunks(self: Vc<Self>) -> Result<Vc<Assets>> {
        let this = self.await?;
        let inner = this.inner.await?;
...
impl ChunkItem for WithClientChunksChunkItem {
    ...
    async fn references(self: Vc<Self>) -> Result<Vc<AssetReferences>> {
        ...
        for &chunk in client_chunks.iter() {
            references.push(Vc::upcast(SingleAssetReference::new(chunk, client_chunk)));
        }
